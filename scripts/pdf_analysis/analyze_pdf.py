...
class PDFAnalyzer:
    ...
    def __init__(self, pdf_path: str, output_dir: str):
        ...
        # 初始化 OCR 引擎
        self.ocr = OCRExtractor(engine="auto")
        if self.ocr.is_available():
            logger.info(f"OCR 引擎已就绪: {self.ocr.engine}")
        else:
            logger.warning("OCR 引擎不可用，将跳过 OCR 处理")
    ...
    def analyze_page(self, page_num: int, renderer: PDFRenderer,
        ...
        # 如果需要 OCR，对渲染图进行 OCR 处理
        ocr_result = None
        if text_quality['need_ocr'] and self.ocr.is_available():
            logger.info(f"  [2.5/6] 执行 OCR 识别...")
            ocr_result = self.ocr.extract_from_image(str(render_file))
            if ocr_result['success']:
                # 保存 OCR 文本
                ocr_text_file = page_dir / "text_ocr.txt"
                with open(ocr_text_file, 'w', encoding='utf-8') as f:
                    f.write(ocr_result['text'])
                logger.info(f"    OCR 成功: {len(ocr_result['text'])} 字符, 置信度: {ocr_result['confidence']:.2%}")
            else:
                logger.warning(f"    OCR 失败: {ocr_result.get('error', '未知错误')}")
...
def main():
    ...
    # PDF 文件路径（容器内路径）
    pdf_files = [
        "/app/data/attachments/00_企业资料说明.pdf",
        "/app/data/attachments/01 氢璞2025产品单页（1023）校正稿.pdf",
        "/app/data/attachments/02（已压缩）氢璞2025宣传册（0905）-解决方案全.pdf"
    ]
